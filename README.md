...
**1. Confidence threshold calibration**: stored in `confidence_thresholds`, per-tenant or global. The workflow fetches them at runtime, and they can be updated through `PUT /confidence-thresholds/:tenant_id` with no redeploy. The cost model for tuning: `override_rate` from correction_events (override_count / total) is the signal. If override_rate for the 0.85-0.90 band is low, raise `auto_approve_min`. If it is high, lower it.
